...
import com.victor.vhealth.protocol.DiseaseKeywordSearchProtocol;
 ...
 */
public class DiseaseKeywordSearchFragment extends KeyWordSearchBaseFragment {


    @ViewInject(R.id.lv_medicine_search)
    private ListView mLvDrug;

    private DiseaseKeywordSearchProtocol mProtocol;
    private List<DiseaseInfo> mDiseaseInfos;

    @Override
    protected LoadingPager.LoadResult initData() {

        mProtocol = new DiseaseKeywordSearchProtocol(mClassifyName, mKeyWord);
        try {
    ...
    protected View initSuccessView() {
        ...
        DiseaseKeySearchAdapter adapter = new DiseaseKeySearchAdapter(mDiseaseInfos, UIUtils.getContext());
    ...
    class DiseaseKeySearchAdapter extends CustomBaseAdapter<DiseaseInfo> {

        public DiseaseKeySearchAdapter(List<DiseaseInfo> datas, Context context) {
            super(datas, context);
        }
        ...
        protected List<DiseaseInfo> onLoadMore() throws Exception {
            return mProtocol.loadData(mDiseaseInfos.size() / 20 + 1);
        }
        ...
        protected void onNormalItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            intent.putExtra(ContentBaseFragment.DATA_ID, (int) mDiseaseInfos.get(position).id);
