...
import com.victor.vhealth.protocol.CompanyDetailProtocol;
 ...
 */
public class CompanyDetailFragment extends ContentBaseFragment {
    ...
    @ViewInject(R.id.tv_company_add)
    private TextView mTvAddress;
    @ViewInject(R.id.iv_company_detail)
    ...
    protected LoadingPager.LoadResult initData() {
        ...
        CompanyDetailProtocol protocol = new CompanyDetailProtocol(Constant.URL.MEDICINE_COMPANY, mId);
        try {
            mCompanyInfo = protocol.loadData(0);
            return checkState(mCompanyInfo);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    protected View initSuccessView() {
        View view = View.inflate(UIUtils.getContext(), R.layout.fragment_company_detail, null);
        ...
        BitmapHelper.display(mIvImg, Constant.URL.IMG_BASE + mCompanyInfo.img);
        mTvName.setText(mCompanyInfo.name);
        mTvType.setText("类型：" + mCompanyInfo.type);
        mTvTel.setText("电话：" + mCompanyInfo.tel);
        mTvAddress.setText("地址：" + mCompanyInfo.address);
        if ("".equals(mCompanyInfo.message)) {
            mTvMessage.setText("暂无简介");
        } else {
            mTvMessage.setText(Html.fromHtml(mCompanyInfo.message));
    ...
    private int getShortHeight() {
        ...
        view.setMaxLines(4);
        view.setText(Html.fromHtml(mCompanyInfo.message));
